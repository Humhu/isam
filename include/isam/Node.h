...
namespace isam {
	...
	public:
		...
		// Return the vector form of the current value
		Eigen::VectorXd vector(Selector s = ESTIMATE) const {return (s==ESTIMATE)?_value->vector():_value0->vector();}
		// Return the vector form of the current linearization point
		...
		// Set the linearization point value
		...
		// Apply the displacement to the current estimate
		void apply_exmap(const Eigen::VectorXd& v) {*_value = _value0->exmap(v);}
		// Apply the displacement to the linearization point
